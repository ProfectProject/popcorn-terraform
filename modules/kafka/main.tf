...
resource "aws_instance" "kafka" {
  ...
  root_block_device {
    ...
    tags = merge(var.tags, {
      Name = "${var.project_name}-kafka-${var.environment}-${count.index + 1}-root"
    })
  }
  ...
  ebs_block_device {
    ...
    tags = merge(var.tags, {
      Name = "${var.project_name}-kafka-${var.environment}-${count.index + 1}-data"
    })
  }
  ...
  tags = merge(var.tags, {
    Name   = "${var.project_name}-kafka-${var.environment}-${count.index + 1}"
    Role   = "kafka"
...
resource "aws_cloudwatch_log_group" "kafka" {
  ...
  tags = merge(var.tags, {
    Name = "${var.project_name}-kafka-${var.environment}-logs"
  })
}